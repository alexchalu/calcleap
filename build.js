...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const toolMap = {};
            if (typeof tools !== 'undefined') tools.forEach(t => toolMap[t.id] = t);
            // Map new IDs to old IDs for backward compat
            const idMap = {
            ...
            const tool = toolMap[lookupId];
            if (tool && tool.render) {
                tool.render(document.getElementById('toolContent'));
            } else {
                // New tool - render from page-specific function
                const fn = window['render_' + toolId.replace(/-/g, '_')];
                if (fn) fn(document.getElementById('toolContent'));
            }
        });
    </script>
</body>
